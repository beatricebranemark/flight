  ...
  const sendData = clickedBar => {
    ...
    //bar-filter
      ...
      organisation_trips.forEach(trip => {
        ...
        if (parseInt(departure[0]) == clickedBar.year) {
          let month_int = parseInt(departure[1])
  ...
  useEffect(() => {
    ...
    g.append('g')
      ...
      .attr('class', function(d) {
        return 'bar_active ' + d.key + ' bar_rect'
      })
      ...
      .on('click', function(d) {
        ...
        if (
          ...
          var bars_inactive = document.getElementsByClassName(
            'bar_inactive'
          )
          var bars_active = document.getElementsByClassName(
            'bar_active'
          )
          if (bars_inactive.length == 0) {
            // if no bars are inactive
            for (var i = 0; i < bars_active.length; i++) {
              let fullClassName = d3
                .select(bars_active[i])
                .attr('class')
              bars_active[i].className.baseVal =
                'bar_inactive ' +
    ...
    var legend = legendContainerSVG
      ...
      .on('click', function(d) {
        ...
        for (var i = 0; i < rects.length; i++) {
          ...
          rects[i].className.baseVal =
            'bar_inactive ' +
            fullClassactive.split(' ')[1] +
            ' bar_rect'
        }
        for (var i = 0; i < barsByYear.length; i++) {
          // set all bars with right year to active
          let fullClassyear = barsByYear[i].className.baseVal
          barsByYear[i].className.baseVal =
            'bar_active ' + fullClassyear.split(' ')[1] + ' bar_rect'
        }
        let obj = {
          month: 'legend',
          year: d,
